...
inline at::Tensor attn(
  ...
  static uint64_t attn_access_time_us = 0;

  if (token == 2) {
    // KV Sizes and addresses
    ...
    // Enqueue actual key tensor address range info
    {
      TensorLayerAddressRange k_addr{};
      std::strncpy(k_addr.name, "K", MAX_NAME_LEN - 1);
      k_addr.name[MAX_NAME_LEN - 1] = '\0';
      k_addr.layer_num = (layer == 0) ? 32 : layer;
      k_addr.start_addr = reinterpret_cast<uintptr_t>(key_start);
      k_addr.end_addr = reinterpret_cast<uintptr_t>(key_end);
      g_tensor_layer_queue.enqueue(k_addr);
    }

    // Enqueue actual value tensor address range info
    {
      TensorLayerAddressRange v_addr{};
      std::strncpy(v_addr.name, "V", MAX_NAME_LEN - 1);
      v_addr.name[MAX_NAME_LEN - 1] = '\0';
      v_addr.layer_num = (layer == 0) ? 32 : layer;
      v_addr.start_addr = reinterpret_cast<uintptr_t>(value_start);
      v_addr.end_addr = reinterpret_cast<uintptr_t>(value_end);
      g_tensor_layer_queue.enqueue(v_addr);
    }

    // Enqueue special end marker "KV dump over"
    if (layer == 0) {
      TensorLayerAddressRange end_marker{};
      std::strncpy(end_marker.name, "dump_over", MAX_NAME_LEN - 1);
      end_marker.name[MAX_NAME_LEN - 1] = '\0';
      end_marker.layer_num = (layer == 0) ? 32 : layer;
      end_marker.start_addr = reinterpret_cast<uintptr_t>(value_start);
      ;
      end_marker.end_addr = reinterpret_cast<uintptr_t>(value_end);
      ;
      g_tensor_layer_queue.enqueue(end_marker);
    }
  }
#endif
...
#pragma omp parallel for collapse(3)
    ...
    attn_access_time_us +=
        std::chrono::duration_cast<Microseconds>(end_attn - start_attn).count();
    std::ofstream latency_log(
        "/data/sathvik/tpp-pytorch-extension/log_files/attn_kernel_latency.log",
        std::ios::app);
    layer = layer ? layer : 32;
    if (latency_log.is_open()) {
      latency_log << "Token " << token << ", Layer: " << layer
                  << ", Attention kernel compute latency: "
                  << attn_access_time_us << " us\n";
      latency_log.close();
    } else {
      std::cerr
          << "Failed to open KV cache access latency log file for writing.\n";
    }
...
//     using Clock = std::chrono::high_resolution_clock;
//     using Microseconds = std::chrono::microseconds;
//     static thread_local uint64_t kv_access_time_us = 0;  // microseconds
...
//         size_t key_bytes = t_KL.numel() * t_KL.element_size();
//         size_t value_bytes = t_VL.numel() * t_VL.element_size();
//         double total_gb = static_cast<double>(key_bytes + value_bytes) /
...
//         // Write to log file
//         std::ofstream
//         kv_log("/data/sathvik/tpp-pytorch-extension/kv_cache_size.log",
//         std::ios::app); if (kv_log.is_open()) {
//             kv_log << "Layer " << layer << ": " << std::endl;
//             kv_log << "Total KV cache size: " << std::fixed <<
//             std::setprecision(6) << total_gb << " GB\n"; kv_log << "Key
//             tensor:   start=" << key_start << "  end=" << key_end << "
//             size=" << key_bytes << " bytes\n"; kv_log << "Value tensor:
//             start=" << value_start << "  end=" << value_end << "  size=" <<
//             value_bytes << " bytes\n"; kv_log << "K.numel(): " <<
//             t_KL.numel() << " and size of each element : " <<
//             t_KL.element_size() << " bytes\n"; kv_log << "V.numel(): " <<
//             t_VL.numel() << " and size of each element : " <<
//             t_VL.element_size() << " bytes\n"; kv_log.close();
//         } else {
...
//                         float omax[qbsr], osum[qbsr], cmax[qbsr], csum[qbsr];
//                         auto S1End = (!isCausal ? S1 : (sq + qbs + offset +
//                         S2 - 1) / S2);
...
//                                 Tc k_buf[S2 * H];
//                                 if (isBeamSearch) {
//                                     auto p_idx = beam_idx ? nullptr :
//                                     nullptr; for (int s2 = 0; s2 < S2; s2++)
//                                     {
...
//                                 ak.a_gemm_tpp(q_ptr, k_ptr, AS[k], 1, true);
//                                 ak.scale_tpp(AS[k], AS[k], one_by_sqrt_H);
//                                 ak.add_mask_tpp(AM ? nullptr : nullptr,
//                                 AS[k]);
//                             }
//                             ak.a_gemm_tpp.release();
...
//                                     for (int k1 = k1start; k1 < k1end; k1++)
//                                     {
//                                         auto lk2start = k1 == k1start ?
//                                         k2start : 0; for (int r = 0; r < R;
//                                         r++) {
//                                             for (int k2 = lk2start; k2 < S2;
//                                             k2++) {
//                                                 AS[k1 - s1][sq1 * S2 * R + r
//                                                 * S2 + k2] = -1e9f;
//                                             }
...
//                             }
//                             if (s1 == 0) {
//                                 ak.softmax_fwd_tpp(kbs, AS[0], AST[0], omax,
//                                 osum, nullptr);
//                             } else {
//                                 ak.softmax_fwd_tpp(kbs, AS[0], AST[0], cmax,
//                                 csum, omax);
//                             }
//                             Tc v_buf[kbs * S2 * H];
//                             T tmp[qbs * R * H];
...
//                                 ak.cvt_tpp(tmp, c_ptr);
//                             } else {
//                                 ak.softmax_fixup(tmp, c_ptr, cmax, csum,
//                                 omax, osum);
//                             }

//                             auto end_kv = Clock::now();
//                             kv_access_time_us +=
//                             std::chrono::duration_cast<Microseconds>(end_kv -
//                             start_kv).count();
...
// {
// std::ofstream
// latency_log("/data/sathvik/tpp-pytorch-extension/kv_cache_access_latency.log",
// std::ios::app); if (latency_log.is_open()) {
//     std::cout << "Latency file is open" << std::endl;
//     latency_log << "Layer: " << layer << ", Token: " << token
//                 << ", KV cache access latency: " << kv_access_time_us << "
//                 us\n";
//     latency_log.close();
// } else {
//     std::cerr << "Failed to open KV cache access latency log file for
//     writing.\n";
// }
